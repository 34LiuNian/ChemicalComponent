...
import { renderMolecules, renderMolecule } from './molecule-renderer.js';
...
export class ReactionScheme {
  ...
  async render(options = {}) {
    ...
    const renderOpts = {
      ...
      disableOverlay: options.disableOverlay,
      aligner,
    };
...
export async function renderReactionScheme(container, steps, options = {}) {
  ...
  // 从 options 中分离出对齐相关参数
  const { aligner, autoAlign, ...rest } = options;
  const renderOpts = { ...rest };
  if (aligner) renderOpts.aligner = aligner;
  if (autoAlign) renderOpts.autoAlign = autoAlign;

  await scheme.render(renderOpts);

  if (options.resize !== false) {
    scheme.enableResize(typeof options.resize === 'number' ? options.resize : 500);
  }
